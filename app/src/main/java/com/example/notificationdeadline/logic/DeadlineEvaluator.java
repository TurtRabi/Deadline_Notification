...
public class DeadlineEvaluator {
    ...
    public Map<Integer, List<NotificationEntity>> evaluateDeadlinesByUrgency() {
        ...
        List<NotificationEntity> all = service.featAllNotifiactionsByDay();
        ...
        for (NotificationEntity entity : all) {
            long diff = entity.timeMillis - now;

            if (diff <= highThreshold && diff>=0) {
                result.get(HIGH_ALERT).add(entity);
            } else if (diff > highThreshold && diff <= mediumThreshold) {
                result.get(MEDIUM_ALERT).add(entity);
            }else if(diff<=lowThreshold && diff>=mediumThreshold){
